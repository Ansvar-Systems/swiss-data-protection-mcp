# ─────────────────────────────────────────────────────────────────────────────
# Swiss Data Protection MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t swiss-data-protection-mcp .
# Run:    docker run --rm -p 3000:3000 swiss-data-protection-mcp
#
# The image bakes /app/data/fdpic.db at build time. Override FDPIC_DB_PATH
...
ENV FDPIC_DB_PATH=/app/data/fdpic.db
...
# Bake the pre-built database into the image so /app/data/fdpic.db resolves
...
# We then COPY it into the image at /app/data/fdpic.db (FDPIC_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match — directory-form `COPY data/` would be
# skipped and the DB would never reach the image.
COPY data/database.db data/fdpic.db
